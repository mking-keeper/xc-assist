...
export interface DestinationResolutionResult {
  /** The resolved destination string ready for xcodebuild */
  destination: string;
  /** Whether the destination was modified during resolution */
  wasResolved: boolean;
  /** Optional warning message if there were issues */
  warning?: string;
  /** Details about what was resolved */
  details?: string;
}
 ...
 */
function parseSimulatorList(output: string): SimulatorDevice[] {
  ...
  for (const line of lines) {
    ...
    const deviceMatch = line.match(
      /^\s+(.+?)\s+\(([A-Za-z0-9-]+)\)\s+\((Shutdown|Booted|Unavailable)\)/i,
    );
...
function findBestMatch(
  ...
  const matches = devices.filter(
    (device) => device.available && device.name === deviceName,
  );
 ...
 */
async function queryAvailableSimulators(): Promise<SimulatorDevice[]> {
  try {
    ...
    throw new Error(
      `Failed to query simulators: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
