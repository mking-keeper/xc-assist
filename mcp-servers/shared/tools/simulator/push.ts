...
export const simulatorPushDefinition: ToolDefinition = {
  name: "simulator_push",
  ...
  inputSchema: {
    ...
    properties: {
      ...
      payload: {
        type: "object",
        description:
          "Push notification payload. Must contain 'aps' key with Apple Push Notification values",
        properties: {
          aps: {
            type: "object",
            ...
            properties: {
              alert: {
                description:
                  "Alert message - string or object with title/body/subtitle",
              },
              badge: {
                type: "number",
...
export async function simulatorPush(
  params: PushParams,
): Promise<ToolResult<PushResultData>> {
  try {
    if (!params.bundle_id) {
      return {
        success: false,
