...
export interface PasteboardParams {
  device_id?: string;
  action: "copy" | "paste";
  text?: string;
}

export interface PasteboardResultData {
  message: string;
  action: "copy" | "paste";
  text?: string;
  note?: string;
  ...
  inputSchema: {
    ...
    properties: {
      ...
      action: {
        ...
        enum: ["copy", "paste"],
        description:
          "'copy' - write text to simulator clipboard, 'paste' - read text from simulator clipboard",
      },
      text: {
        type: "string",
...
export async function simulatorPasteboard(
  params: PasteboardParams,
): Promise<ToolResult<PasteboardResultData>> {
  try {
    ...
    logger.error("Pasteboard operation failed", error as Error);
    return {
      success: false,
      error: String(error),
      operation: "pasteboard",
